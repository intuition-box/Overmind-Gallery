...
export function BidHistory({ tokenId, className = '' }: BidHistoryProps) {
  ...
  if (isLoading) {
    return (
      <div className={`bg-gray-800/20 rounded-xl p-5 border border-gray-700/20 ${className}`}>
        <h3 className="text-lg font-semibold text-white mb-3">Bid History</h3>
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="animate-pulse flex items-center justify-between py-3 border-b border-gray-700 last:border-b-0">
  ...
  if (error) {
    return (
      <div className={`bg-red-900/20 rounded-xl p-5 border border-red-700/30 ${className}`}>
        <h3 className="text-lg font-semibold text-white mb-3">Bid History</h3>
        <p className="text-red-400 text-sm">Failed to load bid history: {error}</p>
      </div>
    )
  }
  ...
  if (bids.length === 0) {
    return (
      <div className={`bg-gray-800/20 rounded-xl p-5 border border-gray-700/20 ${className}`}>
        <h3 className="text-lg font-semibold text-white mb-3">Bid History</h3>
        <p className="text-gray-400 text-sm">No bidding history available for this relic.</p>
      </div>
    )
  }
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-white font-medium text-sm">{bid.bidderName}</span>
                </div>
                <div className="flex items-center gap-2 mt-1">
                  <Clock className="w-3 h-3 text-gray-400" />
                  <span className="text-gray-400 text-xs">{formatDate(bid.timestamp)}</span>
                </div>
                <span className="text-gray-500 text-xs">{formatAddress(bid.bidder)}</span>
              </div>
            </div>
            <div className="text-right">
              <div className="text-white font-medium">{bid.amount} TRUST</div>
